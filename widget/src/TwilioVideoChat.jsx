...
// Configure Twilio Video SDK logging (replaces the deprecated ConnectOptions "logLevel").
Video.Logger.getLogger("twilio-video").setLevel("warn");
var chat;
var joinRoomToggle;
var previewToggle;
var activeRoom;
        ...
        if (typeof chat.participants[participant.sid] != "undefined") {
            ...
            if (chat.currentParticipant.sid == participant.sid) {
                ...
                if (Object.keys(chat.participants).length == 0) {
                    ...
                    //console.warn("-4-");
                    const p = chat.participants[Object.keys(chat.participants)[0]];
                    while (previewContainer.firstElementChild) {
                        previewContainer.firstElementChild.remove();
                    }
                    chat.currentParticipant = null;
...
function showPreview() {
    previewToggle = true;
    var localTracksPromise = localTracks ? Promise.resolve(localTracks) : Video.createLocalTracks();
...
function hidePreview() {
    ...
    if (previewContainer) {
        ...
        if (previewAudio) {
            previewContainer.removeChild(previewAudio);
        }
    }
}
